...
const SolutionParent = () => {
  ...
  const getProductImage = (childTitle: string) => {
    const images = {
      diracenterprise:
        "/products/diracenterprise/diracenterprise.png?height=400&width=600",
    ...
    return (
      images[childTitle as keyof typeof images] ||
      activeChild?.live_img ||
      "/images/EnterpriseEdition.png"
    );
  };
  ...
  const handleTabClick = (index: number) => {
    ...
    if (descriptionRef.current) {
      const y =
        descriptionRef.current.getBoundingClientRect().top + window.scrollY;
      const offset =
        11.25 * parseFloat(getComputedStyle(document.documentElement).fontSize);
      window.scrollTo({
        top: y - 180, // 32px = 2rem
        behavior: "smooth",
      });
  ...
  `}
      ...
      {activeChild && (
        <div
          className="md:container mx-auto px-6 py-12 md:space-y-12"
          ref={descriptionRef}
        >
              ...
              <div className="md:w-1/2 text-left">
                ...
                <div className="text-[#003366] mt-1 leading-relaxed text-justify">
                  {displayLines.map((line, index) => (
                    <div
                      key={index}
                      className="min-h-[1.5em] flex items-center"
                    >
